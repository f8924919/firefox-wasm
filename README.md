...
- **ファイルダウンロードの取り出し** — 上流にはない機能。WASM Firefox 内で
  ダウンロードしたファイルを OPFS 経由で手元のブラウザのダウンロードとして
  受け取れる(`download-bridge.js`。保存先をディスクバックの OPFS に向けるため、
  大容量ファイルでタブがメモリ不足になる問題も同時に回避)
